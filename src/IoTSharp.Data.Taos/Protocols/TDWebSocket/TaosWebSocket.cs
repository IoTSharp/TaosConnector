...
namespace IoTSharp.Data.Taos.Protocols.TDWebSocket
{
    internal partial class TaosWebSocket : ITaosProtocol
    {
        ...
        {
            var rep = WSExecute<WSConnRsp, WSConnReq>(_client, new WSActionReq<WSConnReq>() { Action = "conn", Args = new WSConnReq() { user = builder.Username, password = builder.Password, db = builder.DataBase, req_id = 0 } });
            if (rep.code == 899)
            {
                rep = WSExecute<WSConnRsp, WSConnReq>(_client, new WSActionReq<WSConnReq>() { Action = "conn", Args = new WSConnReq() { user = builder.Username, password = builder.Password, db = builder.DataBase, req_id = 0 } });
            }
            if (rep.code != 0)
            {
